...
Research layer for forecasting and nowcasting UK CPI openings from independently collected predictor data.
...
- `collector_desnz_uk` — DESNZ predictor feeds, beginning with weekly road fuels.
- `collector_defra_uk` — DEFRA predictor feeds, beginning with fruit/vegetable wholesale prices.
...
The original V0.1 collection code is preserved in Git history. Standalone repository trees were prepared on the `split-ready/*` branches before the research-only tree was created, so source ownership can move without losing provenance.
