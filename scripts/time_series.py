...
def _insert_statement(count: int):
    values = ", ".join(
...
def upsert_time_series(
    ...
    Rules:
    1. First sighting inserts vintage_date=collection day.
    2. Identical later values are a no-op.
    3. Changed values on a later UTC day insert a new vintage and are marked as revisions.
    4. Changed values on the same UTC day fail closed. The DATE vintage key cannot
       preserve two intraday information sets, so updating in place would create
       look-ahead for an intraday as-of query.
    """
        ...
        for series_id in batch_ids:
            for observation in by_series[series_id]:
                ...
                if previous_vintage == today:
                    raise RuntimeError(
                        "Point-in-time unsafe same-day revision for "
                        f"{series_id} {observation.reference_date}: existing={current['value']} "
    ...
    logger.info(
        "Time-series upsert: new=%d new_vintages=%d", new_observations, new_vintages
    )
    return WriteResult(
        new_observations,
        new_vintages,
